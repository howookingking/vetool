...
import { X as RemoveIcon, Check, RotateCcw } from 'lucide-react'
import React, {
  KeyboardEvent,
  createContext,
  forwardRef,
...
interface MultiSelectorResetProps
  extends React.ComponentPropsWithoutRef<typeof CommandItem> {
  label?: string
}

const MultiSelectorReset = ({
  className,
  label = '초기화',
  ...props
}: MultiSelectorResetProps) => {
  const { onValueChange, setInputValue } = useMultiSelect()
  ...
  return (
    <CommandItem
      ...
      className={cn(
        'flex cursor-pointer items-center justify-between gap-2 rounded-md px-2 py-1 transition-colors hover:opacity-70',
        className,
      )}
      onMouseDown={mousePreventDefault}
    >
